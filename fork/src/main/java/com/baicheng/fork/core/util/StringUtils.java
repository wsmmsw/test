...
public abstract class StringUtils extends org.apache.commons.lang.StringUtils {
	 ...
	 * of overlapping elements, which are only included on their first occurence).
	 *
	 * @param array1 the first array (can be <code>null</code>)
	 * @param array2 the second array (can be <code>null</code>)
	 * @return the new array (<code>null</code> if both given arrays were
	 ...
	 *            to attempting the split operation (typically the quotation mark
	 ...
	 * 切割全类名，得到类名
	 */
	public static String splitFullClassName(String fullClassName) {
		if (hasLength(fullClassName)) {
			int lastIndexOf = fullClassName.lastIndexOf(".");
			String result = fullClassName.substring(lastIndexOf + 1,
	 ...
	 * 过滤非法字符，用于字符串处理。
	 */
	public static String filterIllegalCharacter(String src) {
		if (src == null) {
			return null;
		}
	 ...
	 * @return
	 */
	public static String removeIllegalCharacter(String src) {
		if (src == null) {
			return null;
		}
		final StringBuilder result = new StringBuilder("");
